@testset "Hermite-Laguerre expansions" begin
    ...
    @testset "Multiply-by-μ" begin
        ...
        T(x) = 1 + 0.1*sin(x)
        f(x, μ) = (μ^2+3)*exp(-μ / T(x))
        μf(x, μ) = μ*(μ^2+3)*exp(-μ / T(x))

        coefs = approximate_f(f, disc, [1, 4], buffer)
        expected = approximate_f(μf, disc, [1, 4], buffer)

        actual = reshape(as_xvx(coefs) * disc.vdisc.Ξμ', size(coefs))
