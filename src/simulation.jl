...
function process_snapshot(t, snapshot, snapshot_index, α::Species, d)
    ...
    jldopen(snapshot_file, "a") do file
        ...
        file["snapshot_$(snapshot_index)_$(α.name)"] = Dict{String, Any}(
            ...
            "Pi_xx" => hostarray(snapshot.Pi_xx),
            "Pi_yy" => hostarray(snapshot.Pi_yy),
            "Pi_zz" => hostarray(snapshot.Pi_zz),
            "Pi_xy" => hostarray(snapshot.Pi_xy),
            "Pi_xz" => hostarray(snapshot.Pi_xz),
            "Pi_yz" => hostarray(snapshot.Pi_yz),
