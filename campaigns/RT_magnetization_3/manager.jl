...
function campaign_sims()
    ...
    rLis = [1.0, 1.5] .* α
    Aes = 1.0 ./ [100, 50, 25]
    T_ref = RTMagnetization3.reference_temp()
    vti = sqrt(T_ref)
    return [(; ωcτ=vti/rLi, Ae) for rLi in rLis, Ae in Aes] |> vec
end
...
function setup_campaign()
    ...
    for (id, params) in enumerate(sims)
        (; ωcτ, Ae) = params
        (; d) = RTMagnetization3.make_sim_vlasov(Val(:gpu); ωcτ, Ae)
...
function run_sim(; id)
    (; ωcτ, Ae) = campaign_sims()[id]
    (; d, sim) = RTMagnetization3.make_sim_vlasov(Val(:gpu); ωcτ, Ae)
    set_simpath(id, d)
    d = PDEHarness.normalize!(d)
    display(d)
    τg = 1 / d["ωg"]
    @show t_end = 80.0 * τg
    dt_omega_c_tau = 0.015 / ωcτ * Ae
    @show dt_omega_c_tau
    dt = dt_omega_c_tau
