...
function campaign_sims()
    ...
    rLis = [0.05, 0.1, 0.2, 0.4, 0.65, 1.0] .* α
    T_ref = RTMagnetization1.reference_temp()
    vti = sqrt(T_ref)
...
function setup_campaign()
    ...
    for (id, params) in enumerate(sims)
        (; ωcτ) = params
        (; d) = RTMagnetization1.make_sim_hybrid(Val(:gpu); ωcτ)
...
function run_sim(; id)
    ...
    (; d, sim) = RTMagnetization1.make_sim_hybrid(Val(:gpu); ωcτ)
    set_simpath(id, d)
